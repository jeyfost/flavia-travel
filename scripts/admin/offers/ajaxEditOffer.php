...
$id = $mysqli->real_escape_string($_POST['offer']);
...
if(!is_numeric($url)) {
    $urlCheckResult = $mysqli->query("SELECT COUNT(id) FROM ft_offers WHERE url = '".$url."' AND id <> '".$id."'");
    $urlCheck = $urlCheckResult->fetch_array(MYSQLI_NUM);

    if($urlCheck[0] == 0) {
        $newsResult = $mysqli->query("SELECT * FROM ft_offers WHERE id = '".$id."'");
        ...
        if($mysqli->query("UPDATE ft_offers SET header = '".$header."', description = '".$description."', text = '".$text."', url = '".$url."' WHERE id = '".$id."'")) {
            echo "ok";
        } else {
            echo "failed";
        }
